...
T=str2num(get(handles.edit19, 'string'));
%11/6/2020 
%generate the inputs header file and generate the values in .c file,
%made by Mariam Hamdy and Mariam Salah, deliverable number 1
fileID2=fopen('input.h','w'); %open .h file and start to define the input variables
fprintf(fileID2,'#ifndef _INPUT_H_\n');
fprintf(fileID2,'#define _INPUT_H_\n');
fprintf(fileID2,'extern const float A[');
fprintf(fileID2, num2str(size(Ap,2)));
fprintf(fileID2,'];\n');
fprintf(fileID2,'extern const float B[');
fprintf(fileID2, num2str(size(Bp,2)));
fprintf(fileID2,'];\n');
fprintf(fileID2,'extern const float w0;\n');
fprintf(fileID2,'extern const float zeta;\n');
fprintf(fileID2,'extern const float Ts;\n');
fprintf(fileID2,'#endif');
fclose(fileID2); %close the .h file
fileID3=fopen('input.c','w'); %open .c file and start to assign the values
fprintf(fileID3,'#include \"input.h"\'); %assign the values of the inputs
fprintf(fileID3,'\n');
fprintf(fileID3,'const float A[]=');
fprintf(fileID3,'{');
for k=1:size(Ap,2) %print the values of the array according to its size
    fprintf(fileID3,[num2str(Ap(k))]);
    if k ~= size(Ap,2)
        fprintf(fileID3,',');
    end
end
...
fprintf(fileID3,'{');
for k=1:size(Bp,2)
    fprintf(fileID3,[num2str(Bp(k))]);
    if k ~= size(Bp,2)
        fprintf(fileID3,',');
    end
end
fprintf(fileID3,'};\n')
fprintf(fileID3,'const float w0=%f;\n',w);
fprintf(fileID3,'const float zeta=%f;\n',zeta);
fprintf(fileID3,'const float Ts=%f;\n',Ts);
fclose(fileID3);
simulate_RST(Ts, Bp, Ap, R, S, T, Bm, Am, dist, handles);
...
%11/6/2020 
%generate the RST header file and the RST values in .c file,
%made by Mariam Hamdy and Mariam Salah, deliverable number 2
fileID = fopen('RST.h','w'); %open .h file and start to define the RST
...
fclose(fileID); %close the .h file
fileID1= fopen('RST.c','w'); %open .c file which contain the values of the RST
fprintf(fileID1,'#include \"RST.h"\');
fprintf(fileID1,'\n');
fprintf(fileID1,'const float R[]=');
fprintf(fileID1,'{');
for k=1:size(R,2) %print the values of the array according to its size
